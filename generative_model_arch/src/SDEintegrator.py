...
def generate_samples(model: torch.nn.Module,
    ...
    If ode_mode=True: dX_t = \hat{b}_t(X_t)dt
    If ode_mode=False: dX_t = \hat{b}_t(X_t)dt + \sqrt{2 D_t^*} dW_t
    """
    X_t = torch.randn(num_samples, ambient_dim, device=device)
    dt = 1.0 / num_time_steps
    time_grid = torch.linspace(0, 1.0 - dt, num_time_steps)
    ...
    for step, t in enumerate(time_grid):
        X_t = X_t.detach() # Explicit detachment to prevent memory leakage
        ...
        # Diagnostic Logging: Monitor the L2 norm of the transport field
        if step % 10 == 0:
        ...
        if ode_mode:
            # Execute deterministic Probability Flow ODE
            X_t = X_t + b_t * dt
        else:
            # Execute standard Euler-Maruyama SDE
            D_t_star = compute_optimal_diffusion(t.item())
