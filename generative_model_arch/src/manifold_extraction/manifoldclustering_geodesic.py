...
class WhitneyPartitionOfUnity:
    ...
    def _bump_function(self, dist_sq: torch.Tensor, r_sq: float) -> torch.Tensor:
        ...
        weights[mask] = torch.clamp(torch.exp(-1.0 / (1.0 - normalized_sq)), min=1e-7)
        return weights
...
def construct_whitney_atlas(data: torch.Tensor, 
    ...
    # Initialize Intrinsic Geodesic Tracker
    geodesic_distances = dijkstra(knn_graph, indices=fps_indices[0], directed=False)
    
    # Initialize Ambient Euclidean Tracker
    current_center = data[fps_indices[0]]
    ambient_distances = torch.norm(data - current_center, p=2, dim=1).cpu().numpy()
    
    
    while True:
        # Mask points already covered intrinsically OR too close extrinsically
        valid_mask = (geodesic_distances > delta_minimax + 1e-7) & (ambient_distances > delta_minimax + 1e-7)
        ...
        # Suppress invalid points from the argmax selection
        masked_geodesic = np.where(valid_mask, geodesic_distances, -np.inf)
        
        # Select the topologically furthest valid point
        farthest_idx = int(np.argmax(masked_geodesic))
        fps_indices.append(farthest_idx)
        
        # 1. Update Intrinsic Bounds
        dist_to_new_geo = dijkstra(knn_graph, indices=farthest_idx, directed=False)
        geodesic_distances = np.minimum(geodesic_distances, dist_to_new_geo)
        
        # 2. Update Ambient Bounds
        new_center = data[farthest_idx]
        dist_to_new_amb = torch.norm(data - new_center, p=2, dim=1).cpu().numpy()
        ambient_distances = np.minimum(ambient_distances, dist_to_new_amb)
    ...
    for i in range(m):
        ...
        resolved_radius = max(delta_minimax, kth_dist * 1.1)
